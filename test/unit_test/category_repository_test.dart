...
void main() {
  ...
  group('CategoryRepositoryImpl.getAll', () {
    test(
      'когда API успешно отдаёт данные, репозиторий возвращает их и сохраняет в локальную базу',
      () async {
        when(
          () => api.getAll(),
        ).thenAnswer((_) async => [incomeCat, outcomeCat]);
    ...
    test(
      'когда API выбрасывает исключение, репозиторий берёт данные из локальной базы',
      () async {
    ...
    test(
      'когда API выбрасывает исключение и нет локальных данных, репозиторий должен вернуть пустой список',
      () async {
        when(() => api.getAll()).thenThrow(Exception('network down'));
  ...
  group('CategoryRepositoryImpl.getByType', () {
    ...
    test(
      'когда нет категорий указанного типа, возвращает пустой список',
      () async {
        when(() => api.getAll()).thenAnswer((_) async => [incomeCat]);
        when(() => dao.clearAndInsertAll(any())).thenAnswer((_) async {});
  ...
  group('CategoryRepositoryImpl edge cases', () {
    test('метод getByType не вызывает ошибку при отсутствии данных', () async {
    ...
    test(
      'выброс исключения, если не удаётся получить данные из базы',
      () async {
        when(() => api.getAll()).thenThrow(Exception('network error'));
        when(() => dao.getAllCategories()).thenThrow(Exception('DB error'));
